...
function isLowStock(p: Product): boolean {
  return Number(p.current_stock) <= Number(p.low_stock_alert)
}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="w-full space-y-4">
            ...
            {fields.map((field, i) => {
              ...
              return (
                <Card key={field.id} className="shadow-sm border-muted-foreground/20 overflow-hidden">
                  <CardHeader className="py-3 px-4 bg-muted/30 border-b flex flex-row items-center justify-between gap-2">
                    <CardTitle className="text-sm font-semibold">Item {i + 1}</CardTitle>
                    {fields.length > 1 && (
                      <Button
                        type="button"
                        variant="ghost"
                          ...
                          {selectedProduct && (
                            <span
                              className={cn(
                                ...
                                isLowStock(selectedProduct) ? 'text-yellow-600' : 'text-green-600'
                              )}
                            >
        ...
        <div className="flex flex-col-reverse sm:flex-row gap-3 pt-2 border-t">
          <Link href="/quotations" className="sm:mr-auto">
