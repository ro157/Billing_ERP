...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [invoices] = await db.execute(
    `SELECT id, invoice_no, date, total_amount, paid_amount, balance_amount, status
     FROM invoices WHERE customer_id = ? ORDER BY date DESC LIMIT 20`,
    [params.id]
  ) as any[]
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (error) return error
  const [rows] = await db.execute('SELECT COUNT(*) as cnt FROM invoices WHERE customer_id = ?', [params.id]) as any[]
