...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  const { error } = await requirePermission('purchase-orders', 'edit')
  if (error) return error
  try {
    const { status, notes } = await req.json()
    await db.execute('UPDATE purchase_orders SET status=COALESCE(?,status), notes=COALESCE(?,notes) WHERE id=?',
      [status || null, notes || null, params.id])
    const [rows] = await db.execute(
      'SELECT po.*, v.name as vendor_name FROM purchase_orders po LEFT JOIN vendors v ON po.vendor_id = v.id WHERE po.id = ?', [params.id]
    ) as any[]
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  const { error } = await requirePermission('purchase-orders', 'delete')
  if (error) return error
  await db.execute('DELETE FROM purchase_order_items WHERE purchase_order_id = ?', [params.id])
  await db.execute('DELETE FROM purchase_orders WHERE id = ?', [params.id])
