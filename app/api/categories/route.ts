...
export async function GET(req: NextRequest) {
  ...
  const [rows] = await db.execute('SELECT * FROM categories ORDER BY name ASC') as any[]
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const id = randomUUID()
    await db.execute('INSERT INTO categories (id, name, description) VALUES (?, ?, ?)', [id, name, description || null])
