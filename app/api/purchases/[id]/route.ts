...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!body.items || !Array.isArray(body.items)) {
      const [rows] = await conn.execute('SELECT * FROM purchases WHERE id = ?', [params.id]) as any[]
      if (!rows[0]) return NextResponse.json({ error: 'Not found' }, { status: 404 })
      ...
      const [updated] = await db.execute(
        'SELECT p.*, v.name as vendor_name FROM purchases p LEFT JOIN vendors v ON p.vendor_id = v.id WHERE p.id = ?',
        [params.id]
      ) as any[]
    ...
    const [existingRows] = await conn.execute('SELECT * FROM purchases WHERE id = ?', [params.id]) as any[]
    ...
    await insertPurchaseItems(conn, params.id, purchaseNo, itemsWithTotals, gstType)
